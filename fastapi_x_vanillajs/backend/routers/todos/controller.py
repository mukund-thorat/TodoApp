...
@router.get("/active", response_model=list[TodoSchema], status_code=HTTP_200_OK)
@limiter.limit(f"{RATE_LIMIT}/minute")
async def get_active_todos(request: Request, limit: int = 15, skip: int = 0, db: AsyncIOMotorDatabase = Depends(get_db), user: UserSchema = Depends(get_current_user)) -> list[TodoSchema]:
    return await fetch_active_todos(user_id=user.userId, limit=limit, skip=skip, db=db)

@router.get("/inactive", response_model=list[TodoSchema], status_code=HTTP_200_OK)
@limiter.limit(f"{RATE_LIMIT}/minute")
async def get_inactive_todos(request: Request, limit: int = 15, skip: int = 0, db: AsyncIOMotorDatabase = Depends(get_db), user: UserSchema = Depends(get_current_user)) -> list[TodoSchema]:
...
@limiter.limit(f"{RATE_LIMIT}/minute")
async def update_title(request: Request, todo_id: str, updated_title: str, db: AsyncIOMotorDatabase = Depends(get_db)) -> ResponseModel:
...
@limiter.limit(f"{RATE_LIMIT}/minute")
async def update_status(request: Request, todo_id: str, status: bool, db: AsyncIOMotorDatabase = Depends(get_db)) -> ResponseModel:
...
@limiter.limit(f"{RATE_LIMIT}/minute")
async def remove_todo(request: Request, todo_id: str, db: AsyncIOMotorDatabase = Depends(get_db)) -> ResponseModel:
